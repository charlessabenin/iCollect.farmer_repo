...
@Component({
  ...
  templateUrl: './edit-household.page.html',
  styleUrls: ['./edit-household.page.scss'],
})
export class EditHouseholdPage implements OnInit {
  ...
  loadData() {
    this.db.getHousehold(this.id).then(household => {
      ...
      let filepath = this.file.externalRootDirectory + 'icollect/household/';
    ...
    this.db.lastLogedUser().then(usr => {
      ...
      .then(async () => {
        ...
        setTimeout(() => {
          ...
          //this.db.syncData();
          this.navCtrl.navigateBack(['/household-details', this.id]); 
        }, 2000);
      });
    });
  }
  ...
  takePicture(sourceType: PictureSourceType) {
    ...
    this.camera.getPicture(options).then(imagePath => {
      ...
      let newPath = this.file.externalRootDirectory + 'icollect/household/';
