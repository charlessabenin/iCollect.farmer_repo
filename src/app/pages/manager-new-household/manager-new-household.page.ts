...
@Component({
  selector: 'app-manager-new-household',
  templateUrl: './manager-new-household.page.html',
  styleUrls: ['./manager-new-household.page.scss'],
})
export class ManagerNewHouseholdPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.id_contact = this.activatedRoute.snapshot.paramMap.get('contact_id');
    ...
    this.db.lastLogedUser().then(usr => {
      ...
      this.db.getNewContactId().then(contact => { 
        ...
        this.db.newHousehold(contact.new_id, this.id_contact, this.fname, this.lname, year, this.relation, this.graduate_primary, this.graduate_secondary, this.graduate_tertiary, this.working_on_farm, this.working_off_farm, usr.id_contact, this.created_date, usr.id_contact, null, null, this.gender, this.read_write, this.schooling, null, null).then(_ => {
          ...
          setTimeout(() => {
            this.navCtrl.navigateBack(['/manager-household-list', this.id_contact]);
          }, 2000);
        });
      });
    });
